...
template<size_t Dimension>
void parallelSimulation(int it, std::vector<Particle<Dimension>>* particles, int dim, double softening, double delta_t, std::string fileName, std::ofstream& file){
...
int main() {
    ...
    // Parallel simulation
    time(&start);
    parallelSimulation<d>(it, &particles, dim, softening, delta_t, fileName, file);
    time(&end);
